<div class="row mb-3 {{ $errors->has($name) ? 'has-danger' : ''}} {{$col ?? null}}">
    ...
    <div class="col-md-4">
    <div class="file-selection-wrapper file-selection-wrapper-{{ $id }} {{ $errors->has($name) ? 'is-invalid' : ''}}">
    </div>
 ...
 <script type="text/javascript">
       ...
       var CustomFile = (function(inputName, idAttr, uploadableFileCount, fileType, disabled) {
          ...
          function fileTemplate(inputName, index, customFileWrapperCount, disabled) {
            ...
            var actioButtons = shouldDisplayButton?'<div style="width: 100%;height: 25px;background: #;text-align: right;display: nsone;padding-right: 10px;padding-top: 3px;">'+
                        '<span index-id="'+index+'" style="line-height: 16px;font-size: 14px;padding: 4px 10px;background: darkred;color: #fff;cursor: pointer;border-radius: 10%;" id="'+removeButton+'">-</span>'+
                        '<span index-id="'+index+'" style="margin-left: 2px;line-height: 16px;font-size: 14px;padding: 4px 8px;background: darkgreen;color: #fff;cursor: pointer;border-radius: 10%;" id="'+addButton+'">+</span>'+
                    '</div>': '';

            return '<div style="margin-bottom: 10px;display: block;" class="custom-file-wrapper-'+idAttr+'" wrap-id="'+idAttr+index+'">'+
                    '<div style="width: calc( 100% - 2px );height: 35px;background: #ced4da;padding:1px;border-top-right-radius: 2px;border-bottom-right-radius: 2px;border-top-left-radius: 2px;border-bottom-left-radius: 2px;">'+
                      '<input type="file" style="opacity: 0;z-index: 3;position: absolute;cursor: pointer;"'+ disabled +'/>'+
                        '<div style="width: 32px;height: 33px;border-right:1px #ced4da solid;background-color: #f5f5f5;float: left;border-top-left-radius: 2px;border-bottom-left-radius: 2px;padding-top: 6px;padding-left: 10px;">'+fileTypeIcon+'</div>'+
                        ...
                        '<div style="width: 108px;height: 33px;padding-top: 1px;background-color: #f5f5f5;float: left;border-left: 1px solid #ced4da;border-right: 1px solid #ced4da;">'+
                            ...
                            '<input type="file" id="'+idAttr+'" name="'+inputName+'" attr="drug-file-input" style="opacity: 0;width: 100%;height: 100%;z-index: 3;position: relative;cursor: pointer;"'+ disabled +' />'+
                        '</div>'+
         ...
         function displayFile() {
            ...
            if( customFileWrapperCount < uploadableFileCount ) {
                for (let index = initialCount; index <= initialCount; index++) {
                    $(document).find(wrapperDiv).append( fileTemplate(inputName, index, customFileWrapperCount, disabled) );
                }
            } else {
       ...
       CustomFile('{{ $name }}', '{{ $id }}', '{{ $uploadableFileCount }}', "{{ strtolower($fileType) }}", "{{ strtolower($disabled) }}");
